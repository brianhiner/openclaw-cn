...
- For **global installs**, the script uses `npm install -g clawdbot-cn@latest` under the hood.
...
Notes:
- `pnpm build` matters when you run the packaged `clawdbot` binary ([`dist/entry.js`](https://github.com/jiulingyun/clawdbot-chinese/blob/main/dist/entry.js)) or use Node to run `dist/`.
...
Tip: to see the current published version, run `npm view clawdbot-cn version`.
